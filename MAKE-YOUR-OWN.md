...
- **Home-screen icon links.** Near the top of `guide.js`'s `boot()` function, three lines point at `jzesbaugh/svg-public` for the manifest and app icon (`manifest.json`, `icons/icon-180.png`). Replace `jzesbaugh/svg-public` in those two URLs with `YOUR_USERNAME/YOUR_REPO`, and swap in your own `manifest.json` + icon files (or delete those lines if you'd rather skip the home-screen-save feature). If you skip this step, visitors who save your fork to their home screen will get our Seattle icon and guide name instead of yours.
